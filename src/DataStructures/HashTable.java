...
// UC1: hashtable class using linkedlistHashMap
public class HashTable<K extends Comparable<K>, V> {
    int number_of_nodes;
    int currently_filled_nodes;
    LinkedListHashMap<K, V>[] hashtable;

    public HashTable() {
        this.number_of_nodes = 100;
        this.hashtable = new LinkedListHashMap[number_of_nodes];
    ...
    private void generate(int size) {
        for (int i = 0; i < size; i++) {
            this.hashtable[i] = new LinkedListHashMap<K, V>();
        }
    }
    ...
    public void remove(K key) {
        int index = this.getIndexforKey(key);
        LinkedListHashMap<K, V> list = hashtable[index];
